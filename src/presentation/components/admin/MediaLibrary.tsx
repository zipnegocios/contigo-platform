...
import { Trash2, Upload, Film, Image, LayoutGrid, Folder, FolderOpen, Info } from 'lucide-react'
...
export function MediaLibrary() {
  ...
  return (
    <>
                ...
                {visibleItems.map((item) => (
                  ...
                  >
                    <div style={{ aspectRatio: '4/3' }}>
                      ...
                      {/* Info button — top-left, visible on hover */}
                      <button
                        onClick={() => setDetailItem(item)}
                        className="absolute top-2 left-2 p-1.5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-200"
      ...
      {detailItem && (
        <MediaDetailsModal item={detailItem} onClose={() => setDetailItem(null)} />
      )}
    </>
  )
}
