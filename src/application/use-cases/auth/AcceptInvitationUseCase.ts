...
export class AcceptInvitationUseCase {
  ...
  async execute(plainToken: string, newPassword: string): Promise<void> {
    ...
    const token = await this.authTokenRepository.findValidByHash(tokenHash, 'invitation')
    if (!token) throw new Error('This invitation link is invalid or has expired')

    const user = await this.adminUserRepository.findById(token.userId)
    if (!user) throw new Error('This invitation link is invalid or has expired')
    ...
    if (this.securityEventLogger) {
      await logSecurityEventSafely(this.securityEventLogger, {
        eventType: SECURITY_EVENT_TYPES.INVITATION_ACCEPTED,
        actorId: user.id,
        payload: {},
      })
    }
